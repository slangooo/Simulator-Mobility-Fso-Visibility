...
from visibility_graph import Point, VisGraph
import numpy as np
...
class IccSimulator:
    ...
    def calculate_selected_vertices(self):
        ...
        self.selected_vertices = []
        for _vertex in self.sunny_vertices + [self.src, self.dest]:
    ...
    def plot_shadows(self):
        if not self._controller.mobility_model.shadow_grid:
    ...
    def plot_sunny_vertices(self):
        for idx, _vertex in enumerate(self.sunny_vertices):
            if not idx:
                plt.plot(_vertex[0], _vertex[1], c='red', marker='*', label='Sunny location', linestyle='none')
            else:
                plt.plot(_vertex[0], _vertex[1], c='red', marker='*')

    def plot_obstacles(self):
        self._controller.mobility_model.graph.obstacles_objects.plot_obstacles(False)
    ...
    def plot_shortest_path(self):
        ...
        for idx in range(len(path) - 1):
            ...
            if not idx:
                plt.plot(xs, ys, c='red', label='FSO link')
            else:
    ...
    def plot_endpoints(self):
        ...
        plt.plot(self.dest[0], self.dest[1], c='green', marker='o', linestyle='none', label='Hotspot center')
...
# [print(_bs.energy_level) for _bs in simulator._controller.base_stations]
    ...
    ax.legend(['Without RES', 'With RES'], loc="upper left")
    ax.set_xlabel('hour')
    ax.set_ylabel('# of new DBS arrivals')
    ...
    ax2 = ax.twinx()
    width = 0.2
    ax.bar(a.transpose(), simulator.average_energy_per_hour_final.flatten(), color='r', width=width)
    ax.bar(a.transpose() + width, simulator_with_energy.average_energy_per_hour_final.flatten(), color='y', width=width)
    ax2.plot(elev_angles, color='b')
    ax.set_xticks(a)
    ax.set_xticklabels(a, rotation=65)
