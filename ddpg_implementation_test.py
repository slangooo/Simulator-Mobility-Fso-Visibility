...
if __name__ == '__main__':
    ...
    def get_actor():
        ...
        angle_margin = 10  # degrees

        # Our upper bound is 2.0 for Pendulum.
        # outputs = outputs * (360 - angle_margin) / 360 if outputs > 0 else (360 + outputs * angle_margin)/360
    ...
    def policy(state, noise_object):
        sampled_actions = tf.squeeze(actor_model(tf.reshape(state, (1, num_states))))
        noise = noise_object()
    ...
    for ep in range(total_episodes):
        ...
        while True:
            ...
            tf_prev_state = tf.expand_dims(prev_state, 0)

            action = tf.convert_to_tensor(policy(tf_prev_state, ou_noise))
            # Recieve state and reward from environment.
            time_step = env.step(action)
            state, reward, done = time_step.observation, time_step.reward, time_step.is_last()
...
t_step = env.reset()
action = tf.convert_to_tensor([tf.squeeze(actor_model(tf.reshape(t_step.observation, (1, num_states))))])
t_step = env.step(action)
